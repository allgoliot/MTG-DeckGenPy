    ...
    with ui.header().classes('bg-gradient-to-r from-purple-600 to-blue-600'):
        with ui.row().classes('w-full justify-between items-center'):
            with ui.row().classes('items-center gap-2'):
                ui.button('🎴 MTG DeckGenPy', on_click=lambda: ui.navigate.to('/')).props('flat color=white').classes('text-xl font-bold')
            ...
            with ui.row().classes('w-full gap-6'):
                ...
                with ui.column().classes('flex-1 min-w-[400px]'):
                    ...
                    with ui.row().classes('w-full gap-2 mb-4'):
                        search_input = ui.input(placeholder='Rechercher un commandant...').classes('flex-grow')
                    ...
                    commander_container = ui.column().classes('w-full max-h-[400px] overflow-y-auto')
                    commanders_data = []
                            ...
                            with container:
                                ...
                                with container:
                                    ...
                                    for idx, cmd in enumerate(data):
                                        ...
                                        with ui.row().classes(row_classes).on('click', select_cmd):
                                            ui.label(f"{cmd['name']}{tribes_str}").classes('flex-grow')
                                            ui.label(f"B{cmd['bracket_group']}").classes('bg-blue-100 text-blue-800 text-xs px-2 py-1 rounded')
                            ...
                            with preview_container:
                                ...
                                with preview_container:
                                    ...
                                    with ui.column().classes('w-full items-center gap-3'):
                                        ...
                                        ui.label(f"🎯 {cmd['name']}").classes('text-lg font-bold text-center')
                                        ui.label(f"🎨 {engine.format_colors(cmd['colors_parsed'])}").classes('text-3xl')

                                        if cmd.get('tribes'):
                                            with ui.row().classes('gap-1 flex-wrap justify-center'):
                                                for tribe in cmd['tribes']:
                                                    ui.badge(tribe.capitalize()).classes('bg-purple-100 text-purple-800')
                        except RuntimeError:
                            pass
                ...
                try:
                    with tribe_container:
                        ...
                        with tribe_container:
                            ...
                            tribe_grid = ui.grid().classes('grid-cols-6 gap-2 max-h-64 overflow-y-auto w-full')
                            checkboxes = {}
...
def library_page():
    ...
    with ui.column().classes('w-full max-w-7xl mx-auto p-6 gap-6'):
        ...
        async def load_library():
            ...
            await asyncio.sleep(0.1)  # Laisser le temps à l'UI de se mettre à jour

            # Charger les decks avec progression
                ...
                with deck_grid:
                    with ui.card().classes('w-full cursor-pointer hover:shadow-lg transition-shadow').on('click', lambda f=deck_file: show_deck_details(f)):
                        ...
                        with ui.column().classes('w-full items-center p-4'):
                            if image_url:
                                ui.image(image_url).classes('w-full max-h-80 object-contain rounded-lg')
                            else:
            ...
            with loading_card:
                ...
                with loading_card:
                    ...
                    ui.button('🔄 Actualiser', on_click=lambda: asyncio.create_task(load_library())).classes('mt-2')
        ...
        ui.timer(0.05, start_loading, once=True)
